...
// ─── Update User Status (subadmin limited) ────────────────────────────────────
export const updateUserStatusBySubadmin = async (req: Request, res: Response) => {
  try {
    const userId = Number(req.params.id);
    if (isNaN(userId)) return errorResponse(res, "Invalid user ID", 400);
    ...
    // Subadmin can only set inactive (enforced by middleware); revoke active sessions.
    if (status !== "active") {
      await removeAllUserTokensForUserId(userId);
    }

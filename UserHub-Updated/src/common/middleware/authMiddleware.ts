...
export const authenticate: RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    if (userTokenRow) {
      ...
      // Role.USER is only used internally so admin-only routes (which call allowRoles)
      // reject user sessions. User routes skip allowRoles entirely.
      authReq.user = { id: Number(decoded.id), role: Role.USER };
    }
